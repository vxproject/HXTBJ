...
Page({
  ...
  countDown: function (e) {
    ...
    that.msTime(that.data.list_data.flashSale[0])
  ...
  msTime: function (item) {
    ...
    let is_miaosha = false,flag = 0
    let time_obj = {}, miaoshaFlag = false
    if (item.end_time) {
      ...
      if (timestamp > end_time) {
        ...
        if (timestamp < item.start_time){
          time = item.start_time - timestamp
          flag = 1
        }else{
          //秒杀中
        ...
        if (time > 0) {
          ...
          if (flag == 1){
            ...
            //刷新页面数据
            setTimeout((res) => {
              that.getRequest(this.data.goods_id)
            }, 1000)
